...
func TestOverlappingBlocksDetectsAllOverlaps(t *testing.T) {
	...
	// will handle that.
	var blocks = make([]*Block, 11)
	for i := 10; i >= 0; i-- {
		blocks[i] = &Block{meta: BlockMeta{MinTime: int64(i * 10), MaxTime: int64((i + 1) * 10)}}
	}

	testutil.Assert(t, len(OverlappingBlocks(blocks)) == 0, "we found unexpected overlaps")
	...
	add := func(bs ...*Block) []*Block {
		repl := append(append([]*Block{}, blocks...), bs...)
		sort.Slice(repl, func(i, j int) bool {
			return repl[i].Meta().MinTime < repl[j].Meta().MinTime
		})
		return repl
	}

	// o1 overlaps with 10-20.
	o1 := &Block{meta: BlockMeta{MinTime: 15, MaxTime: 17}}
	testutil.Equals(t, Overlaps{
		{Min: 15, Max: 17}: {blocks[1], o1},
	}, OverlappingBlocks(add(o1)))

	// o2 overlaps with 20-30 and 30-40.
	o2 := &Block{meta: BlockMeta{MinTime: 21, MaxTime: 31}}
	testutil.Equals(t, Overlaps{
		{Min: 21, Max: 30}: {blocks[2], o2},
		{Min: 30, Max: 31}: {o2, blocks[3]},
	}, OverlappingBlocks(add(o2)))

	// o3a and o3b overlaps with 30-40 and each other.
	o3a := &Block{meta: BlockMeta{MinTime: 33, MaxTime: 39}}
	o3b := &Block{meta: BlockMeta{MinTime: 34, MaxTime: 36}}
	testutil.Equals(t, Overlaps{
		{Min: 34, Max: 36}: {blocks[3], o3a, o3b},
	}, OverlappingBlocks(add(o3a, o3b)))

	// o4 is 1:1 overlap with 50-60.
	o4 := &Block{meta: BlockMeta{MinTime: 50, MaxTime: 60}}
	testutil.Equals(t, Overlaps{
		{Min: 50, Max: 60}: {blocks[5], o4},
	}, OverlappingBlocks(add(o4)))

	// o5 overlaps with 60-70, 70-80 and 80-90.
	o5 := &Block{meta: BlockMeta{MinTime: 61, MaxTime: 85}}
	testutil.Equals(t, Overlaps{
		{Min: 61, Max: 70}: {blocks[6], o5},
		{Min: 70, Max: 80}: {o5, blocks[7]},
		{Min: 80, Max: 85}: {o5, blocks[8]},
	}, OverlappingBlocks(add(o5)))

	// o6a overlaps with 90-100, 100-110 and o6b, o6b overlaps with 90-100 and o6a.
	o6a := &Block{meta: BlockMeta{MinTime: 92, MaxTime: 105}}
	o6b := &Block{meta: BlockMeta{MinTime: 94, MaxTime: 99}}
	testutil.Equals(t, Overlaps{
		{Min: 94, Max: 99}:   {blocks[9], o6a, o6b},
		{Min: 100, Max: 105}: {o6a, blocks[10]},
	}, OverlappingBlocks(add(o6a, o6b)))

	// All together.
	testutil.Equals(t, Overlaps{
		{Min: 15, Max: 17}: {blocks[1], o1},
		{Min: 21, Max: 30}: {blocks[2], o2}, {Min: 30, Max: 31}: {o2, blocks[3]},
		{Min: 34, Max: 36}: {blocks[3], o3a, o3b},
		{Min: 50, Max: 60}: {blocks[5], o4},
		{Min: 61, Max: 70}: {blocks[6], o5}, {Min: 70, Max: 80}: {o5, blocks[7]}, {Min: 80, Max: 85}: {o5, blocks[8]},
		{Min: 94, Max: 99}: {blocks[9], o6a, o6b}, {Min: 100, Max: 105}: {o6a, blocks[10]},
	}, OverlappingBlocks(add(o1, o2, o3a, o3b, o4, o5, o6a, o6b)))

	// Additional case.
	var nc1 []*Block
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 1, MaxTime: 5}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 2, MaxTime: 3}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 2, MaxTime: 3}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 2, MaxTime: 3}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 2, MaxTime: 3}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 2, MaxTime: 6}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 3, MaxTime: 5}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 5, MaxTime: 7}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 7, MaxTime: 10}})
	nc1 = append(nc1, &Block{meta: BlockMeta{MinTime: 8, MaxTime: 9}})
	testutil.Equals(t, Overlaps{

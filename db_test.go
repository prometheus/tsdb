...
func TestTombstoneCleanFail(t *testing.T) {
	...
	for i := 0; i < totalBlocks; i++ {
		blockDir := MockCreateBlock(t, db.Dir(), MockGenSeries(1, 1, 0, 1))
...
func (c *mockCompactorFailing) Write(dest string, b BlockReader, mint, maxt int64, parent *BlockMeta) (ulid.ULID, error) {
	...
	block, err := OpenBlock(nil, MockCreateBlock(c.t, dest, MockGenSeries(1, 1, 0, 1)), nil)
	testutil.Ok(c.t, err)
...
func TestTimeRetention(t *testing.T) {
	...
	for _, m := range blocks {
		MockCreateBlock(t, db.Dir(), MockGenSeries(10, 10, m.MinTime, m.MaxTime))
	}
...
func TestSizeRetention(t *testing.T) {
	...
	for _, m := range blocks {
		MockCreateBlock(t, db.Dir(), MockGenSeries(100, 10, m.MinTime, m.MaxTime))
	}
...
func TestInitializeHeadTimestamp(t *testing.T) {
	...
	t.Run("existing-block", func(t *testing.T) {
		...
		MockCreateBlock(t, dir, MockGenSeries(1, 1, 1000, 2000))
	...
	t.Run("existing-block-and-wal", func(t *testing.T) {
		...
		MockCreateBlock(t, dir, MockGenSeries(1, 1, 1000, 6000))
...
func TestNoEmptyBlocks(t *testing.T) {
	...
	t.Run("Test no blocks remaining after deleting all samples from disk.", func(t *testing.T) {
		...
		for _, m := range blocks {
			MockCreateBlock(t, db.Dir(), MockGenSeries(2, 2, m.MinTime, m.MaxTime))
		}
	...
	for _, c := range cases {
		if ok := t.Run("", func(t *testing.T) {
			...
			for _, series := range c.blockSeries {
				MockCreateBlock(t, tmpdir, series)
			}
			opts := *DefaultOptions
...
func TestBlockRanges(t *testing.T) {
	...
	MockCreateBlock(t, dir, MockGenSeries(1, 1, 0, firstBlockMaxT))
	...
	MockCreateBlock(t, dir, MockGenSeries(1, 1, secondBlockMaxt+1, thirdBlockMaxt))
...
func TestDBReadOnly(t *testing.T) {
	...
	// Boostrap the db.
	{
		...
		for _, m := range dbBlocks {
			MockCreateBlock(t, dbDir, MockGenSeries(1, 1, m.MinTime, m.MaxTime))
		}
		expSeriesCount++
	}
